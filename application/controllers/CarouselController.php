...
class CarouselController extends CI_Controller
{
    ...
    function upload_image(){
        $config['upload_path'] = './uploads/carousel/'; //path folder
        ...
        if(!empty($_FILES['file']['name'])){
            ...
            if ($this->upload->do_upload('file')){
                ...
                $config['source_image']='./uploads/carousel/'.$gbr['file_name'];
                $config['create_thumb']= FALSE;
                $config['maintain_ratio']= true;
                // $config['quality']= '100%';
                //$config['width']= 600;
                // $config['height']= 400;
                $config['new_image']= './uploads/carousel/'.$gbr['file_name'];
                $this->load->library('image_lib', $config);
                $this->image_lib->resize();
 
                $gambar=$gbr['file_name'];

                return base_url('uploads/carousel/'.$gambar);
            }                      
        }else{
            return base_url('uploads/carousel/default.jpg');
        }         
    }
}
